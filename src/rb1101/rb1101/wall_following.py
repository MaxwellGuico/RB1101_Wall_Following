...
class WallFollowingNode(Node):
    ...
    '''Perform a left 90° turn while moving forward (arc). 
    forward_speed_fraction scales translate_velocity (0..1).'''

    # def turn_left_90deg(self, forward_speed_fraction:float=0.1):
    #     '''Perform a left 90° turn while moving forward (arc). 
    #     forward_speed_fraction scales translate_velocity (0..1).'''
    #     self.get_logger().debug("Turn left 90deg (arc)")
    #     turn_velocity = self.get_parameter('turn_velocity').value  # expected in rad/s
    #     translate_velocity = self.get_parameter('translate_velocity').value * np.clip(forward_speed_fraction, 0.0, 1.0)
    #     angle_rad = np.deg2rad(90.0)
    #     # Duration = angle / angular_speed. Guard against zero velocity.
    #     if abs(turn_velocity) < 1e-6:
    #         self.get_logger().warning("turn_velocity is zero, performing in-place sleep fallback")
    #         time.sleep(1.0)
    #     else:
    #         duration = abs(angle_rad / turn_velocity)
    #         # perform the arc in small steps to allow smooth publishing
    #         step_dt = 0.05
    #         steps = max(1, int(np.ceil(duration / step_dt)))
    #         dt = duration / steps
    #         for _ in range(steps):
    #             self.move_2D(translate_velocity, 0.0, turn_velocity)
    #             time.sleep(dt)
    #     self.get_logger().debug("Turn complete")
    #     self.cooldown = 5
    #     self.last_scan = None

    # def turn_right_90deg(self, forward_speed_fraction:float=0.1):
    #     '''Perform a right 90° turn while moving forward (arc). 
    #     forward_speed_fraction scales translate_velocity (0..1).'''
    #     self.get_logger().debug("Turn right 90deg (arc)")
    #     turn_velocity = self.get_parameter('turn_velocity').value  # expected in rad/s
    #     translate_velocity = self.get_parameter('translate_velocity').value * np.clip(forward_speed_fraction, 0.0, 1.0)
    #     angle_rad = np.deg2rad(90.0)
    #     if abs(turn_velocity) < 1e-6:
    ...
    #         dt = duration / steps
    #         for _ in range(steps):
    #             # negative angular for right turn
    #             self.move_2D(translate_velocity, 0.0, -turn_velocity)
    #             time.sleep(dt)
    #     self.get_logger().debug("Turn complete")
    #     self.cooldown = 5
    ...
    def turn_left_90deg(self, lateral_distance:float=None, speed_fraction:float=1.0):
        '''Strafe left (holonomic) without rotating. Moves left by lateral_distance (m).
        If lateral_distance is None, defaults to robot length + 0.15 m.'''
        # Replace fixed-distance strafe with continuous strafe-until-wall for speed
        safe_distance = 0.12  # metres; stop when left sensor reads <= this. Tune as needed.
        timeout = 4.0         # seconds max to avoid infinite loops
        self.strafe_left_until_wall(safe_distance=safe_distance, speed_fraction=speed_fraction, timeout=timeout)

    def strafe_left_until_wall(self, safe_distance:float=None, speed_fraction:float=1.0, timeout:float=4.0):
        """Continuously strafe left (positive linear.y) until left LIDAR sees a wall within safe_distance.
        Publishes at short intervals for a smooth continuous motion."""
        self.get_logger().debug("Strafe left until wall")
        translate_velocity = self.get_parameter('translate_velocity').value * np.clip(speed_fraction, 0.0, 1.0)
        if abs(translate_velocity) < 1e-6:
            self.get_logger().warning("translate_velocity is zero, aborting strafe")
            return

        if safe_distance is None:
            dims = self.get_parameter('robot_dimensions').value
            safe_distance = dims[0] + 0.05

        start = time.time()
        # left-sector indices (approx 80..100 degrees); adjust if your scan indexing differs
        left_slice = np.arange(80, 101)  # inclusive of 100

        y_speed = abs(translate_velocity)  # positive y -> left in this frame
        period = 0.02  # 20 ms loop period; tune for smoother/faster publishes

        while time.time() - start < timeout:
            # If no scan available, publish a brief command and wait for scan
            if self.last_scan is None or self.last_scan.size == 0:
                self.move_2D(0.0, y_speed, 0.0)
                time.sleep(period)
                continue

            # handle wrap / scan size differences defensively
            max_idx = self.last_scan.size - 1
            indices = left_slice[left_slice <= max_idx]
            if indices.size == 0:
                # fallback: check the entire scan if indexing mismatch
                min_left = np.min(self.last_scan)
            else:
                min_left = np.min(self.last_scan[indices])

            # stop if we've reached the desired proximity
            if min_left <= safe_distance:
                break
        ...
        # stop and set cooldown so the main loop can re-evaluate
        self.stop()
        self.cooldown = 5
        self.last_scan = None
        self.get_logger().debug("Strafe left finished (hit wall or timeout)")
